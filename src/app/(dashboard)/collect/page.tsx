...
export default function CollectPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const prompt = `You are an expert in waste management and recycling. Analyze this image and provide with high accuracy:
        1. Confirm if the waste type matches: ${selectedTask.wasteType}
        2. Estimate if the quantity matches: ${selectedTask.amount}
          - A match means the estimate falls within an acceptable range (±5 units).
          -If unsure, but the quantity looks very similar, you may still consider it a match.
      ...
      console.log("Raw Gemini response for report page:", text)
      ...
      try {
        ...
        if (parsedResult.wasteTypeMatch && parsedResult.quantityMatch && parsedResult.confidence > 0.7) {
          ...
          console.log('saveCollectedWaste has executed successfully')
          
          const save_reward = await saveReward(user.id, earnedReward)
          console.log(`Reward: ${save_reward.reward}, Create Reward: ${save_reward.create_reward}, Create Notification: ${save_reward.create_notification}`)
